...
class Archiver(object):
    ...
    def handle_post_save(self, sender, **kwargs):
        #Params: sender=model class, instance=instance saved, created=true if created, 
        #   update_fields=set of fields to update explicitly in save(), using = db alias
        op = UPDATE_OP
    ...
    def handle_post_delete(self, sender, **kwargs):
        #Params: sender=model class, instance=instance deleted, using = db alias 
    ...
    def worker(self):
        while True:
            try:
                ...
                logging.error(ex) #log the exception, but continue on. Don't want this process to die...


    def destroy(self):
        #if destroy not called, a daemon process will terminate when parent process dies.         
        super(QueuedArchiver, self).destroy()
        self.q.put((None, None)) #tells worker to terminate gracefully
        self.p.join() #waits for process to end
        self.writer.destroy()
